...
export default async function handler(req, res) {
  ...
  try {
    ...
    const initialPrompt = `
      ...
      Proszę odpowiedz na następujące pytanie: "Jakie są kluczowe wstępne obserwacje, spostrzeżenia, potencjalne obszary do dalszej analizy oraz hipotezy dotyczące tematu '${topicDisplayName}', biorąc pod uwagę dostarczone podsumowanie danych oraz ogólną naturę zbioru danych?"
      ...
      - "initialFindings": (String) Główna, szczegółowa część Twojej odpowiedzi na powyższe pytanie. Powinny to być Twoje kluczowe wstępne obserwacje, spostrzeżenia lub hipotezy, przeznaczone do wyświetlenia w głównym bloku analizy. Ten ciąg znaków powinien być sformatowany za pomocą tagów HTML dla akapitów (np. "<p>Spostrzeżenie 1.</p><p>Spostrzeżenie 2.</p>"). Kiedy odnosisz się do nazw kolumn z podsumowania danych (np. OperatorWorkload_%, TasksCompleted), NIE używaj odwrotnych apostrofów. Zamiast tego, otocz dokładną nazwę kolumny tagiem <span class="column-name-highlight"></span>. Na przykład, jeśli odnosisz się do 'OperatorWorkload_%', zapisz to jako <span class="column-name-highlight">OperatorWorkload_%</span>. WAŻNE: Cała wartość ciągu znaków dla "initialFindings" musi być prawidłowym ciągiem JSON. Oznacza to, że wszelkie cudzysłowy (") będące częścią treści tekstowej lub atrybutów w HTML (w tym atrybutu class w tagu span) MUSZĄ być poprzedzone znakiem ucieczki jako \\". Na przykład, jeśli akapit to '<p>Powiedział "Więcej danych!".</p>', powinien być reprezentowany w ciągu jako "<p>Powiedział \\"Więcej danych!\\".</p>".
      - "thoughtProcess": (String) Krótko wyjaśnij kroki lub rozumowanie, które doprowadziły Cię do sformułowania odpowiedzi w "initialFindings". Wspomnij, które części podsumowania danych były najbardziej istotne. Ten ciąg znaków powinien być sformatowany jako nieuporządkowana lista HTML (np. "<ul><li>Krok pierwszy wyjaśniający \\"dlaczego\\".</li><li>Krok drugi.</li><li>Krok trzeci.</li></ul>") zawierająca dokładnie 3 punkty. Kiedy odnosisz się do nazw kolumn, NIE używaj odwrotnych apostrofów. Zamiast tego, otocz dokładną nazwę kolumny tagiem <span class="column-name-highlight"></span>, jak opisano powyżej. WAŻNE: Cała wartość ciągu znaków dla "thoughtProcess" musi być prawidłowym ciągiem JSON. Oznacza to, że wszelkie cudzysłowy (") będące częścią treści tekstowej lub atrybutów w elementach listy HTML (w tym atrybutu class w tagu span) MUSZĄ być poprzedzone znakiem ucieczki jako \\".
    ...
    // initialAnalysisResult now contains conciseInitialSummary, initialFindings, thoughtProcess, questionSuggestions
    ...
    await chatHistoryRef.doc(firstMessageId).set({
      role: "model",
      parts: [{ text: initialAnalysisResult.conciseInitialSummary }], // Use concise summary for chat bubble
      timestamp: finalTimestamp, 
      detailedAnalysisBlock: initialAnalysisResult, // Store the full result here, including HTML initialFindings
    ...
    // The 'data' returned to frontend is the full initialAnalysisResult object
    // which now includes 'conciseInitialSummary' along with 'initialFindings', etc.
    return res.status(200).json({
      success: true,
